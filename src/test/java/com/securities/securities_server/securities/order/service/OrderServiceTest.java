...
class OrderServiceTest {
    ...
    void 주문을_생성하고_거래소에_전송한_뒤_체결_결과를_반영하고_응답을_반환한다() {
        ...
        given(exchangeOrderService.placeOrder(exchangeOrderRequest)).willReturn(exchangeOrderResponse);
        ...
        verify(exchangeOrderService).placeOrder(exchangeOrderRequest);
    ...
    void 거래소에_주문_취소_요청을_전송한_뒤_결과를_반영하고_응답을_반환한다() {
        ...
        given(order.getId()).willReturn(orderId);
        given(order.getStock()).willReturn(stock);
        ...
        given(exchangeOrderService.cancelOrder(exchangeCancelRequest))
                .willReturn(exchangeOrderResponse);
        ...
        verify(exchangeOrderService).cancelOrder(exchangeCancelRequest);
        verify(orderResultService).handleExchangeOrderResponse(exchangeOrderResponse, orderId);
    }
